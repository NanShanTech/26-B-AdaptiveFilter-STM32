# 1 "../Tasks/sample_app_task_scheduler.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 344 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../Tasks/sample_app_task_scheduler.c" 2
# 1 "../Tasks/sample_app_task_scheduler.h" 1
...
# 4 "../Tasks/sample_app_task_scheduler.h" 2
# 13 "../Tasks/sample_app_task_scheduler.h"
typedef struct
{
    uint16_t w1msCount;
    uint16_t w10msCount;
    uint16_t w100msCount;
    uint16_t w1secCount;
} TimerData_TypeDef;
...
void SampleAppTaskScheduler(void)
{
    ...
    if (wNumTicks > 0)
    {
        ...
        if (TimerData.w1msCount >= (10U))
        {
            TimerData.w10msCount += (TimerData.w1msCount / (10U));
            TimerData.w1msCount %= (10U);
            SampleAppTask10ms();


            if (TimerData.w10msCount >= (10U))
            {
                TimerData.w100msCount += (TimerData.w10msCount / (10U));
                TimerData.w10msCount %= (10U);
                SampleAppTask100ms();


                if (TimerData.w100msCount >= (10U))
                {
                    TimerData.w100msCount %= (10U);
                    SampleAppTask1sec();
                }
            }
        }
