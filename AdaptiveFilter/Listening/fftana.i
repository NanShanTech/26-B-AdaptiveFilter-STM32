# 1 "../SignalProcess/fftana.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 344 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../SignalProcess/fftana.c" 2
...
arm_cfft_instance_f32_extra *get_fft_num(uint16_t fft_n) {
    switch (fft_n) {
    case 16:
...
void fft_process(fftin *data,fftdata *output) {
    arm_cfft_f32_extra(get_fft_num(8192),data -> cmp,0,1);
...
freqaxis_t *get_freqaxis() {
    __attribute__((section(".RAM_D1"))) static freqaxis_t freqaxis;
    for (uint32_t i = 0; i < 4096; i++) {
        float32_t temp = (float32_t)i;
        freqaxis.axis[i] = temp * 40960 / 8192;
    }
    return &freqaxis;
}
...
void regurlize_mag(fftdata *fft_result,const float32_t regurlize_num) {
    ...
    for (uint16_t i = 0; i < 4096; i++) {
        fft_result ->mag[i] = fft_result -> mag[i] / max_mag * regurlize_num ;
    }
}
    ...
    uint32_t Len, float32_t winCorrFactor, float32_t *pPreciseVpp)
{
    ...
    if (peakIdx < 2 || peakIdx >= (fftMagHalfLen - 2) || fftMag == 0) return 0.0f;
    ...
    if (fabsf(denominator) < 1e-12f) return 0.0f;
    ...
    float32_t windowResponse = 0.54f + 0.46f * cosf(2.0f * 3.14159265358979f * normalizedFreq);
    preciseAmp *= (winCorrFactor / windowResponse);

    if (pPreciseVpp != 0) *pPreciseVpp = preciseAmp * 2.0f;
    return preciseFreq;
}
...
float32_t Max_Three_Find(float32_t* Input, uint16_t Max_Index) {
    ...
    if (target >= 4096) return 0;
    float32_t max_temp = Input[target];

    for(int offset = -2; offset <= 2; offset++) {
        if((target + offset < 4096) && (Input[target + offset] > max_temp))
...
void Phase_atan(float32_t* FFT_In_Complex, uint32_t Index, float32_t* Phase) {
    *Phase = atan2f(FFT_In_Complex[2*Index+1], FFT_In_Complex[2*Index]) * 180.0f / 3.14159265358979f;
}
