# 1 "../MyDrive/Get_Freq.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 344 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../MyDrive/Get_Freq.c" 2
...
static void Freq_Hardware_Switch(uint8_t target_mode) {
    ...
    if(target_mode == 0) {
        ...
        (((EXTI_TypeDef *) (((0x40000000UL) + 0x18000000UL) + 0x0000UL))->PR1 = (((uint16_t)0x0020)));
        ((EXTI_TypeDef *) (((0x40000000UL) + 0x18000000UL) + 0x0000UL))->PR1 = 0xFFFFFFFF;


        HAL_TIM_Base_Stop(&htim2);
        HAL_TIM_IC_Stop(&htim2, 0x00000000U);
        (((RCC_TypeDef *) (((0x40000000UL) + 0x18020000UL) + 0x4400UL))->APB1LRSTR) |= ((0x1UL << (0U)));
        ...
        HAL_TIM_IC_Start(&htim2, 0x00000000U);
        ((&htim2)->Instance->SR = ~((0x1UL << (1U))));
...
static float Get_Capture_Freq(void) {
    uint32_t Rise_Count = HAL_TIM_ReadCapturedValue(&htim2, 0x00000000U) + 1;
    if (Rise_Count <= 1) return 0;
...
void FreqMeasure_Count_Handler(uint16_t GPIO_Pin) {
    if(GPIO_Pin == ((uint16_t)0x0020) && FreqCtrl.current_mode == 1) {
        FreqCtrl.count++;
    }
}
